...
function ViewCard() {
  ...
  let [title,setTitle] = useState(cardDetails.title)
       ...
       let{updating,setUpdating} = useContext(listingDataContex)
       let{deleting,setDeleting} = useContext(listingDataContex)
  ...
  return (
    <div className="w-full min-h-screen bg-white flex items-center justify-center overflow-auto">
      <div className="relative w-full max-w-6xl flex flex-col items-center gap-4 py-6">

        {/* Back Button */}
        <button
          className="w-12 h-12 bg-[red] rounded-full flex items-center justify-center absolute top-4 left-4 z-10"
          onClick={() => navigate("/")}
        >
        ...
        {/* Title */}
        <h1 className="w-[95%] md:w-[80%] text-xl md:text-2xl font-semibold text-[#272727] truncate px-[70px]">
        ...
        <div className="w-[95%] md:w-[80%] flex flex-col md:flex-row gap-4">
          {/* Main Image */}
          <div className="flex-1 h-64 md:h-[400px] border-2 bg-red-500 flex items-center justify-center overflow-hidden">
            <img src={cardDetails.image1} alt="Main" className="w-full h-full object-cover" />
          </div>

          {/* Side Images */}
          <div className="flex-1 flex flex-col gap-4 h-64 md:h-[400px]">
            <div className="flex-1 border-2 flex items-center justify-center overflow-hidden bg-black">
              <img src={cardDetails.image2} alt="Side 1" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 border-2 flex items-center justify-center overflow-hidden bg-black">
              <img src={cardDetails.image3} alt="Side 2" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>

        {/* Details */}
        <div className="w-[95%] md:w-[80%] space-y-2 text-base md:text-lg text-[#272727]">
          <p className="font-semibold">
            {`${cardDetails.title.toUpperCase()} ${cardDetails.category.toUpperCase()}, ${cardDetails.landMark.toUpperCase()}`}
          </p>
          <p className="text-gray-800">{cardDetails.description.toUpperCase()}</p>
          <p>{`Rs. ${cardDetails.rent}/day`}</p>
        </div>

        {/* Edit Button */}
      <div className="w-[95%] md:w-[80%] flex justify-start gap-4 mt-4">
  {cardDetails.host === userData._id ? (
    <button className="px-6 md:px-16 py-2 bg-red-600 text-white text-lg rounded-lg text-nowrap" onClick={()=>setUpdatePopUp(prev => !prev)}>
      Edit Listing
    </button>
  ) : (
    <button className="px-6 md:px-16 py-2 bg-red-600 text-white text-lg rounded-lg text-nowrap"  onClick={()=>setBookingPopUp(prev => !prev)}>
      Reserve
    </button>
  )}
</div>
      ...
      {/* Update Listing page */}
        {updatePopUp && <div className='w-[100%] h-[100%] flex items-center justify-center bg-[#000000c6] absolute top-[0px] z-[100] backdrop-blur-sm'>
           <RxCross2  className="w-6 h-6 bg-[red] rounded-full flex items-center justify-center absolute top-4 left-4 z-10 "
          ...
          <form action="" className='max-w-[900px] w-[90%] h-[550px] flex items-center justify-start flex-col gap-[10px] overflow-auto mt-[50px] text-white bg-[#272727] p-[20px] rounded-lg ' onSubmit={(e)=>{e.preventDefault()}}>
                  ...
                  <div className='w-[90%] flex items-start justify-start flex-col gap-[10px] '>
                        ...
                        <input type="text" id='title' className='w-[90%] h-[40px] border-[2px] border-[#555656] rounded-lg text-[18px] px-[20px] text-[black]' required onChange={(e)=>setTitle(e.target.value)} value={title} />
                      </div>
                 ...
                 <div className='w-[90%] flex items-start justify-start flex-col gap-[10px] '>
                  ...
                  <div className='w-[90%] flex items-start justify-start flex-col gap-[10px] '>
                        ...
                        <input type="number" id='rent' className='w-[90%] h-[40px] border-[2px] border-[#555656] rounded-lg text-[18px] px-[20px] text-[black] '  onChange={(e)=>setRent(e.target.value)} value={rent}/>
                      </div>
                  ...
                  <div className='w-[90%] flex items-start justify-start flex-col gap-[10px] '>
                      ...
                      <div className='w-[100%] flex items-center justify-center gap-[30px] mt-[20px]'>
                       <button className='px-[10px] py-[10px] bg-[red] text-[white] text-[15px] md:text-[18px] md:px-[100px] rounded-lg text-nowrap' onClick={handleUpdateListing} disabled={updating}> {updating?"updating...":"Update Listing"}</button>

                       <button className='px-[10px] py-[10px] bg-[red] text-[white] text-[15px] md:text-[18px] md:px-[100px] rounded-lg text-nowrap' onClick={handleDeleteListing} disabled={deleting}> {deleting?"deleting...":"Delete Listing"}</button>
                      </div>
                  </form>
          ...
          {bookingPopUp && <div className='w-[100%] h-[100%] flex items-center justify-center flex-col gap-[30px]  bg-[white] absolute top-[0px] z-[100] p-[20px] backdrop-blur-sm md:flex-row md:gap-[100px]'>
              <RxCross2  className="w-6 h-6 bg-[red] rounded-full flex items-center justify-center absolute top-4 left-4 z-10 "
          onClick={() =>setBookingPopUp(false)}/>
              <div className=''>
                
              </div>
              <form className='max-w-[450px] w-[90%] h-[450px] overflow-auto bg-[#f7fbfcfe] p-[20px] rounded-lg flex items-center justify-center flex-col gap-[10px] border-[1px] border-[#dedddd]' onSubmit={(e)=>{
                e.preventDefault()
              }}>
                <h1 className='w-[100%]flex items-center justify-center py-[10px] text-[25px] border-b-[1px] border-[#a3a3a3]'>Confirm & Book</h1>
                 ...
                 <div className='w-[100%] h-[70%] mt-[10px] rounded lg p-[10px] '>
                        ...
                        <div className='w-[90%] flex items-start justify-start  gap-[24px] mt-[20px] md:justify-center flex-col md:flex-row md:items-start'>
                        <label htmlFor="checkIn" className='text-[18px] md:text-[20px]'>CheckIn</label>
                        <input type="date" min={minDate} id='checkIn' className='w-[200px] h-[40px] border-2 border-[#555656] bg-transparent rounded-[10px] text-[15px] px-[10px] text-[black] md:text-[18px]' required onChange={(e)=>setCheckIn(e.target.value)} value={checkIn} />
                      </div>

                       <div className='w-[90%] flex items-start justify-start  gap-[10px] mt-[40px] md:justify-center flex-col md:flex-row md:items-start'>
                        <label htmlFor="checkOut" className='text-[18px] md:text-[20px]'>CheckOut</label>
                        <input type="date" min={minDate} id='checkOut' className='w-[200px] h-[40px] border-2 border-[#555656] bg-transparent rounded-[10px] text-[15px] px-[10px] text-[black] md:text-[18px]' required onChange={(e)=>setCheckOut(e.target.value)} value={checkOut}  />
